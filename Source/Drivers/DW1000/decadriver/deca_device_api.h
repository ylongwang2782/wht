...
//! TX_FCTRL register (i.e. a four bit value destined for bits 20..18 but
//! shifted left by 2 for byte alignment)
...
#define DWT_IDLE_ON_DLY_ERR \
    2    // If delayed RX failed due to "late" error then if this
         // flag is set the RX will not be re-enabled immediately, and device
         // will be in IDLE when function exits
#define DWT_NO_SYNC_PTRS \
    4    // Do not try to sync IC side and Host side buffer pointers when
         // enabling RX. This is used to perform manual RX re-enabling when
...
#define DWT_CONFIG \
    0x0040    // ONW_LDC - on wakeup restore (load) the saved configurations
              // (from AON array into HIF)
...
// DWT_DW_POWER_ON should be used when dwt_initialise is called on cold power up
// of DW IC When DW IC is being initialised after wake up then some of the init
// steps can be skipped DW1000 INIT configuration parameters
#define DWT_LOADNONE \
    0x00    // no loading of micro-code or reading of OTP values
#define DWT_LOADUCODE \
    0x01    // this can be called on power up or after wake up to load ucode
#define DWT_DW_WAKE_UP \
    0x02    // init after wake up - will not load ucode / ucode will not run
#define DWT_DW_WUP_NO_UCODE \
    0x04    // init after wake up - ucode has not already been loaded / ucode is
            // not used
#define DWT_DW_WUP_RD_OTPREV \
    0x08    // init after wakeup - read OTP rev after wake up
...
typedef struct {
    ...
    uint8_t
        rxPAC;    //!< Acquisition Chunk Size (Relates to RX preamble length)
    uint8_t txCode;      //!< TX preamble code
    uint8_t rxCode;      //!< RX preamble code
    uint8_t nsSFD;       //!< Boolean should we use non-standard SFD for better
                         //!< performance
    uint8_t dataRate;    //!< Data Rate {DWT_BR_110K, DWT_BR_850K or DWT_BR_6M8}
    uint8_t phrMode;     //!< PHR mode {0x0 - standard DWT_PHRMODE_STD, 0x3 -
...
typedef struct {
    ...
    uint16_t
        OVER;    // number of receiver overflows (used in double buffer mode)
...
/*
 * From version 5.0.0:
 *  - dwt_getinitxtaltrim: Replaced by the function dwt_getxtaltrim which
 * returns current xtal trim value
 *
 * From version 4.0.0:
 ...
 * depending on the RX mode the user wants to set up.
 ...
 * anymore, this functions has become useless.
 *  - dwt_getrangebias: Range bias correction values are platform dependent and
 * should therefore be managed at user application level.
 ...
 * dependent and should therefore be managed at user application level.
 *  - dwt_readantennadelay: Antenna delay values and location in OTP memory are
 * platform dependent and should therefore be managed at user application level.
 ...
 *
 * NOTE: dwt_initialise() must be called prior to this function so that it can
 * return a relevant value (stored in OTP).
 *
 * input parameters
 *
 * output parameters
 ...
 * dependent, consult the DW1000 User Manual. This can also be used for debug as
 * enabling TX and RX GPIOs is quite handy to monitor DW1000's activity.
 *
 ...
 *
 * @brief This is used to senable GPIO clocks. The clocks are needed to ensure
 ...
 * @fn dwt_getgpiovalue()
 *
 * @brief This is used to get GPIO value, returns (1) or (0) depending if the
 * GPIO is high or low
 *
 * input parameters
 * @param gpioNum    -   this is the GPIO to configure - see DWT_GxP0...
 * DWT_GxP8
 *
 * output parameters
 *
 ...
 * for its use and initialises as necessary any static data items belonging to
 * this low-level driver.
 *
 * This function does not need to be called after DW1000 device is woken up from
 * DEEPSLEEP, the device will preserve register values e.g. LDO, UCODE, XTAL.
 * However if needed this function can be called to initialise internal
 * structure  dw1000local[] if it has not been preserved (e.g. if micro was in
 * sleep and its RAM data (containing dw1000local structure was not preserved
 * during sleep)
 *
 * NOTES:
 * 1. When DW1000 is powered on this function needs to be run before
 * dwt_configuresleep, also the SPI frequency has to be < 3MHz
 ...
 *                  DWT_LOADNONE         0x00 - do not load any values from OTP
 * memory DWT_LOADUCODE        0x01 - load the LDE microcode from ROM - enable
 * accurate RX timestamp DWT_DW_WAKE_UP       0x02 - just initialise
 * dw1000local[] values (e.g. DW1000 has woken up) DWT_DW_WUP_NO_UCODE  0x04 -
 * if microcode/LDE algorithm has not already been loaded (on power up) e.g.
 * when LDE is not used DWT_READ_OTP_PID     0x10 - read part ID from OTP
 ...
 *  - Use default OPS table should be used with following register
 * modifications: These modifications optimise the default OPS configuration
 * further for 64 length preamble use case
 *
 * NOTE: These register settings are not preserved during SLEEP/DEEPSLEEP, thus
 * they should be programmed again after wake up
 *
 * input parameters
 * @param prf
 *
 ...
 * max is 1023 standard PHR mode allows up to 127 bytes if > 127 is programmed,
 * DWT_PHRMODE_EXT needs to be set in the phrMode configuration see
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int dwt_writetxdata(uint16_t txFrameLength, uint8_t *txFrameBytes,
 ...
 * immediate TX (response expected - so the receiver will be automatically
 * turned on after TX is done) if mode = DWT_START_TX_DELAYED |
 * DWT_RESPONSE_EXPECTED - delayed TX (response expected - so the receiver will
 ...
 * (depending on the mode parameter). In the case of a "late" error the receiver
 * will only be turned on if the DWT_IDLE_ON_DLY_ERR is not set. The receiver
 * will stay turned on, listening to any messages until it either receives a
 * good frame, an error (CRC, PHY header, Reed Solomon) or  it times out (SFD,
 * Preamble or Frame).
 *
 * input parameters
 ...
 * triggers, then the RX will be enabled immediately (DWT_START_RX_DELAYED |
 * DWT_IDLE_ON_DLY_ERR) 3 used to disable re-enabling of receiver if delayed RX
 * failed due to "late" error (DWT_START_RX_IMMEDIATE | DWT_NO_SYNC_PTRS) 4 used
 * to re-enable RX without trying to sync IC and host side buffer pointers,
 * typically when performing manual RX re-enabling in double buffering mode
 *
 * returns DWT_SUCCESS for success, or DWT_ERROR for error (e.g. a delayed
 ...
 * (on/off) is specified through the parameters below. See DW1000 User Manual
 ...
 * SLEEP state but wakes periodically, (after this "long sleep"), for a very
 * short time to sample the air for a preamble sequence. This preamble sampling
 * "listening" phase is actually two reception phases separated by a "short
 * sleep" time. See DW1000 User Manual section "Low-Power Listening" for more
 * details.
 *
 ...
 * parameter should at least have DWT_PRESRV_SLEEP, DWT_CONFIG and DWT_RX_EN set
 * and "wake" parameter should at least have both DWT_WAKE_SLPCNT and DWT_SLP_EN
 * set.
 ...
 * (DWT_INT_RFCG) only. When configured, low-power listening mode can be
 * triggered either by putting the DW1000 to sleep (using dwt_entersleep()) or
 * by activating reception (using dwt_rxenable()).
 *
 *       Please refer to the low-power listening examples (examples 8a/8b
 * accompanying the API distribution on Decawave's website). They form a working
 * example code that shows how to use low-power listening correctly.
 *
 * input parameters:
 ...
 * 512/19.2 �s (~26.7 �s). The counter automatically adds 1 to the value set.
 * The smallest working value that should be set is 1, i.e. giving a snooze time
 * of 2 units (or ~53 �s).
 *
 * output parameters
 *
 ...
 * listening scheme is implemented. It will only process/report the RXFCG event
 * (through cbRxOk callback). It clears RXFCG interrupt and reads received frame
 * information and frame control before calling the callback.
 *
 ...
 * @param operation - if set to 1 the interrupts (only the ones selected in the
 * bitmask) are enabled else they are cleared
 *                  - if set to 2 the interrupts in the bitmask are forced to
 * selected state - i.e. the mask is written to the register directly.
 *
 * output parameters
 *
 * no return value
 ...
 * operational parameter set table for tight xtal offsets (<1ppm)
 ...
 * @fn dwt_getxtaltrim()
 *
 * @brief This function returns current value of XTAL trim. If this is called
 * after dwt_initalise it will return the OTP value if OTP value is non-zero or
 * FS_XTALT_MIDRANGE if OTP value is zero (not programmed).
 *
 * input parameters
 *
 * output parameters
 ...
 * called before call to this to ensure pdw1000local->tempP contains the
 ...
 *
 * @brief  this function takes in an externally measured temperature in 10ths of
 * degrees Celcius and applies the conversion factor to give a value in IC
 * temperature units, as produced by the SAR A/D. The dwt_initialise needs to be
 * called before call to this to ensure pdw1000local->tempP contains the
 * SAR_LTEMP value from OTP.
 *
 * input parameters:
 * @param externaltemp - this is the an externally measured temperature in 10ths
 * of degrees Celcius to convert
 *
 * output parameters:
 *
 ...
 * call to this to ensure pdw1000local->vBatP contains the SAR_LVBAT value from
 * OTP
 *
 * input parameters:
 ...
 *
 * @brief  this function takes in a true voltage in millivolts and applies the
 * conversion factor to give a raw DW IC value. The dwt_initialise needs to be
 * called before call to this to ensure pdw1000local->vBatP contains the
 * SAR_LVBAT value from OTP.
 *
 * input parameters:
 ...
 *
 * @brief this function determines the corrected bandwidth setting (PG_DELAY
 * register setting) of the DW1000 which changes over temperature.
 *
 * input parameters:
 ...
 *
 * @brief this function determines the corrected power setting (TX_POWER
 setting) for the
 * DW1000 which changes over temperature.
 *
 * Note: only ch2 or ch5 are supported, if other channel is used - the COMP
 factor should be calculated and adjusted
 *
 * input parameters:
 * @param channel - uint8_t - the channel at which compensation of power level
 will be applied: 2 or 5
 * @param ref_powerreg - uint32_t - the TX_POWER register value recorded when
 reference measurements were made
 * @param delta_temp - int - the difference between current ambient temperature
 (raw value units)
 *                                  and the temperature at which reference
 measurements were made (raw value units)

 * output parameters: None
 *
 * returns: (uint32_t) The corrected TX_POWER register value
 */
uint32_t dwt_calcpowertempadj(uint8_t channel, uint32_t ref_powerreg,
                              int delta_temp);
 ...
 *
 * input parameters:
 * @param pgdly - uint8_t - the PG_DELAY to set (to control bandwidth), and to
 * find the corresponding count value for output parameters: None
 *
 * returns: (uint16_t) PGC_STATUS count value calculated from the provided
 * PG_DELAY value - used as reference for later bandwidth adjustments
 */
 ...
 * bit-7 (or with 0x80) for write operation c. if extended sub address index is
 * used (i.e. if index > 127) set bit-7 of the first sub-index byte following
...
void dwt_writetodevice(
    uint16_t recordNumber,    // input parameter - ID of register file or buffer
                              // being accessed
    uint16_t index,     // input parameter - byte index into register file or
                        // buffer being accessed
    uint32_t length,    // input parameter - number of bytes being written
    const uint8_t *buffer    // input parameter - pointer to buffer containing
...
void dwt_readfromdevice(
    ...
    uint8_t *buffer    // input parameter - pointer to buffer in which to return
                       // the read data.
);
 ...
 * define the two low level abstract functions to write to and read from the SPI
 * the definitions should be in deca_spi.c file. Low level abstract function to
 * write to the SPI Takes two separate byte buffers for write header and write
 * data returns 0 for success, or -1 for error
 *
 ...
 * output parameters
 *
 * returns DWT_SUCCESS for success (and the position in the buffer at which data
 * begins), or DWT_ERROR for error
 */
...
//     choose to use #defines here to map these calls transparently to the
//     target system.  Alternatively the appropriate code may be embedded in the
//     functions provided in the deca_irq.c file.
//
// ---------------------------------------------------------------------------
 ...
 * of a critical section It returns the IRQ state before disable, this value is
 * used to re-enable in decamutexoff call
 *
 ...
 * state as returned(&saved) by decamutexon This is called at the end of a
 * critical section
 *
 * Note: The body of this function is defined in deca_mutex.c and is platform
 * specific
